...
namespace EncryptionTest.Tests
{
    public class Tests
    {
        ...
        public async Task TEST_DB_FUNCTION()
        {
            ...
            foreach (Testings testings in testObjs)
            {
                var obj = await _dbContext.Testings.AsNoTracking().FirstOrDefaultAsync(x => EF.Functions.Decrypt(x.encrypted) == testings.encrypted);
                Assert.NotNull(obj);
                Assert.Equal(obj.encrypted,testings.encrypted);
            }
        }
        ...
        public async Task TEST_DB_FUNCTION_EXTENSION()
        {
            ...
            foreach (Testings testings in testObjs)
            {
                var obj = await _dbContext.Testings.AsNoTracking().FirstOrDefaultAsync(x => x.encrypted.MySqlDecrypt() == testings.encrypted);
                Assert.NotNull(obj);
                Assert.Equal(obj.encrypted,testings.encrypted);
            }
        }
        ...
        public async Task TEST_GROUP_BY()
        {
            ...
            var grouped = await _dbContext.Testings.AsNoTracking().GroupBy(x => x.encrypted).Select(x => x.Key)
                .ToListAsync();
            //Test
        ...
        public async Task TEST_MAPPING_DECRYPTED()
        {
            ...
            {
                Id = x.Id,
                encrypted = x.encrypted.MySqlDecrypt().ToUpper(),
                normal = x.normal
            }).ToListAsync();
        ...
        public async Task TEST_DB_FUNCTION_ENCRYPT()
        {
            ...
            foreach (Testings testings in testObjs)
            {
                var obj = await _dbContext.Testings.AsNoTracking().FirstOrDefaultAsync(x => x.encrypted == x.normal.MySqlEncrypt() && x.Id == testings.Id);
                Assert.NotNull(obj);
                Assert.Equal(obj.encrypted,testings.encrypted);
            }
        }
        ...
        public async Task TEST_ENCRYPTION_NULL_VALUE_DB_FUNCTION()
        {
            ...
            await _dbContext.SaveChangesAsync();
            //Validation
            var fromDbObj = await _dbContext.Testings.AsNoTracking().Where(x=> string.IsNullOrEmpty(x.encrypted.MySqlDecrypt())).ToListAsync();
            Assert.Equal(10,fromDbObj.Count);
            Assert.True(fromDbObj.All(x=> string.IsNullOrEmpty(x.encrypted)));
        }
        [Fact]
        public async Task TEST_ENCRYPTION_EMPTY_SPACE_DB_FUNCTION()
        {
            ...
            await _dbContext.SaveChangesAsync();
            //Validation
            var fromDbObj = await _dbContext.Testings.AsNoTracking().Where(x=> string.IsNullOrWhiteSpace(x.encrypted.MySqlDecrypt())).ToListAsync();
            Assert.Equal(10,fromDbObj.Count);
